...
| Logging | Normal | Record normal diagnostic events. | Goldberry/Hardlist clear |
| Logging | Verbose | Record normal and verbose diagnostic events. | Goldberry/Hardlist clear |
| Logging | Diagnostic | Record normal, verbose, and diagnostic events. | Goldberry/Hardlist clear |
| Logging | Trace | Record maximum diagnostic events. | Goldberry/Hardlist clear |
...
| Import Setup | Whole | Import all portable setup state from the selected `.akr` setup pack while preserving machine-local values. This replaces every saved StartPos slot for the pack's target map. | Normal clear |
| Import Setup | StartPos | Import only StartPos setup data from the selected `.akr` setup pack. This replaces every saved StartPos slot for the pack's target map. | Normal clear |
