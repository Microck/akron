...
|---|---|---|---|
| FPS Bypass | Numeric toggle | Raise the draw cadence while keeping the configured simulation cadence when possible. | Cheat |
...
|---|---|---|---|
| FPS Bypass | Target FPS | Draw target while FPS Bypass is enabled. Interval mode rounds to a clean multiple of TPS. | Cheat |
| FPS Bypass | Method | Interval keeps render cadence aligned to physics ticks. Dynamic allows arbitrary FPS but is riskier for code that hooks the main tick. | Cheat |
| FPS Bypass | Smooth Camera | Fancy is highest quality, Fast is cheaper but can jitter backgrounds, Off leaves the camera pixel-locked. | Cheat |
| FPS Bypass | Objects | Extrapolate predicts between physics frames. Interpolate uses prior frames and can add 1-2 frames of delay. | Cheat |
...
| FPS Bypass | Subpixel Madeline | Reference option for drawing Madeline and held items at subpixel render positions. | Cheat |
| FPS Bypass | Smooth background | Reference option for high-resolution background compositing in Fancy camera smoothing. | Cheat |
| FPS Bypass | Smooth foreground | Reference option for high-resolution foreground compositing in Fancy camera smoothing. | Cheat |
| FPS Bypass | Hide edge gaps | Reference option: slightly zooms the level to hide gaps introduced by fractional camera offsets. | Cheat |
