---
title: Custom labels
description: Reference for custom HUD label variables, expressions, events, placement, and shareable .akr packs.
---

Custom labels let you add text rows to the Akron HUD. Use them for local status, timers, routing notes, input state, or other information that is useful while playing or reviewing a run.

Custom HUD labels are **Cheat** because templates can expose exact player position, speed, stamina, dash count, configuration flags, and other internal state. That information can affect how an attempt is played.

For setup steps, see [HUD and input tools](/player-guide/hud-and-input-tools#create-a-custom-label).
...
| `savestate_slot` | Active Speedrun Tool savestate slot. |
...
| `OnNoclipDeath` | Show while the latest visible death marker comes from a death that happened with Noclip on. |
