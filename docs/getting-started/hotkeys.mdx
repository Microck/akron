...
Actions marked `-` have no default binding on that input type. Open Menu starts on `Tab`; Click Teleport and Cursor Zoom use `Left Alt` as their hold input. Other shortcut actions start unbound so you can assign them intentionally.
