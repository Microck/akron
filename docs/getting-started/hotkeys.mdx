...
Actions marked `-` have no default binding on that input type. Open Menu starts on `Tab`; Click Teleport, Cursor Zoom, and Cursor Tools use `Left Alt` as their hold input. Other shortcut actions start unbound so you can assign them intentionally.
...
| Cursor tools | Keep Cursor Tools near Click Teleport and Cursor Zoom if you use the bundled hold workflow. |
