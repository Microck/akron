...
Akron uses Everest's keyboard and controller configuration surfaces for binds. Use those surfaces when possible so bindings stay consistent with the rest of your modded Celeste setup.
...
|---|---|---|
| Toggle Overlay | `Tab` | — |
| Fast Lookout Hold | `Left Shift` | — |
| Retry | `R` | `Y` |
| Reload Room | `F4` | `LB` |
| Open Debug Map | `F5` | `RB` |
| Reload Chapter | `F11` | `Start` |
| Save State | `F7` | `X` |
| Load State | `F8` | `B` |
| Previous Slot | `F9` | `DPad Left` |
| Next Slot | `F10` | `DPad Right` |
| Cycle Grab Mode | `G` | `RStick` |
| Freeze Gameplay | `F1` | — |
| Step Frame | `F2` | — |
| Decrease Timescale | `-` | — |
| Increase Timescale | `+` | — |
...
| Next StartPos | `E` | — |
| Toggle Hitboxes | `F3` | — |
| Toggle Entity Inspector | `Home` | — |
| Click Teleport | `Left Alt` | — |
| Cursor Zoom | `Left Alt` | — |

Actions with a dash (—) have no default binding on that input type. StartPos set, load, and clear actions start unbound so you can assign them intentionally.
