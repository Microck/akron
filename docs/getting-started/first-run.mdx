...
<Steps>
  ...
  <Step title="Check the current setup">
    Look at the overlay status area for the current setup and attempt status. Fresh installs should show no active Akron profile, no primary ruleset, and no attempt classification unless you changed Akron's Everest mod options or imported a setup pack.
  </Step>
...
Use the default setup if you are exploring Akron for the first time. It keeps the normal overlay workflow available without enabling an Akron profile, ruleset, proof workflow, or HUD surface.
