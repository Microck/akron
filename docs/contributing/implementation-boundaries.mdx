...
Use this reference when a change spans UI, settings, policy, runtime behavior, `.akr` archives, commands, HUD, or tests. Put new code in the closest existing owner so each change stays focused.
...
| Community packs | `Source/Community/akron-community-packs.cs`, `Source/Community/akron-community-pack-browser.cs` |

## Adding a feature

Define the behavior before naming its UI row.
...
## `.akr` archive boundaries

`Source/Packs/akron-archive.cs` owns the shared ZIP manifest, entry validation, size checks, and payload helpers. Each archive kind owns its payload schema and import/export behavior:

| Archive kind | Owner |
|---|---|
| Setup | `Source/Setups/akron-setup-packs.cs` |
| Overlay theme | `Source/Overlay/akron-overlay-themes.cs` |
| Custom HUD labels | `Source/Hud/akron-custom-hud-label-catalog.cs` |
| Control display | `Source/Input/akron-input-board.cs` |

Keep OS-specific setup file-picker process launching in `Source/Setups/akron-setup-file-picker.cs`.
...
`Source/Overlay/akron-overlay-popup-controls.cs` is a containment boundary for the current option-popup catalog. Move a popup family into a focused partial when it grows into a full editor, browser, or feature-family surface.
...
| Retry, reload, save/load actions, prompts, and automation action wrappers | `Source/Module/akron-module-actions.cs` |
...
|---|---|
| Setup capture | `Source/Setups/akron-setup-settings.cs` |
| Setup application | `Source/Setups/akron-setup-state-apply.cs` |
| Setup state structure and defaults | `Source/Setups/akron-setup-state.cs` |
| Status formatting, streamer-mode path display, binding labels, low-distraction channels, hitbox style reset, and native menu entries | `Source/Module/akron-module-settings-display.cs` |
...
| Native savestate hook patching and Everest compatibility | `Source/SaveLoad/akron-native-savestate-support.cs` |
| Entity and map risk/safety analysis | `Source/SaveLoad/akron-savestate-risk-analyzer.cs` |
...
| Setup import, export, and sections | `Source/Commands/akron-setup-commands.cs` |
...
- A Cheat suboption inherits a clean parent classification.
