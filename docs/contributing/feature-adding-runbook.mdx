---
title: Feature development runbook
description: Add Akron features with the expected shape, placement, copy, tests, docs, and live verification evidence.
---

Use this runbook when adding a user-facing Akron feature. Start with the player-visible behavior, then choose the smallest UI control that expresses it.
...
A feature row should have one primary behavior. Submenu controls configure that behavior.
...
Pick the tab users would check first. Then follow that tab's existing task groups and workflow order.

Do not append a new row to the end by default.

| Tab | Add features here when | Ordering rule |
|---|---|---|
| `Player` | The feature directly changes Madeline, player resources, movement, or player-attached information. | Grouped as resources, practice assists, HUD controls, then player presentation. |
...
| `StartPos` | The feature captures, restores, selects, or configures persistent StartPos state. | Follow the capture, placement, selection, and respawn workflow. |
| `Backups` | The feature creates, restores, reports, schedules, or retains save backups. | Follow the create, restore, status, trigger, and retention workflow. |
| `Bypass` | The feature changes completion, unlock, or berry state. | Group actions by the save or progression state they change. |
| `Keybinds` | A bindable action needs a read-only binding overview. | Add the behavior to its owning tab, then add its binding overview in the matching workflow order. |
...
| `Creator` | The feature helps route building, map navigation, room capture, debug-map work, or entity inspection. | Keep inspection tools, room navigation, checkpoint/map navigation, then capture and export actions together. |
| `Labels` | The feature draws persistent label-style HUD information. | Uses label row order and user row-order behavior. |
| `Global` | The feature changes Akron-level behavior across scenes. | Keep time and simulation controls first, then attempt policy and persistence controls. |
| `Interface` | The feature changes menu appearance, search, setup import/export, keybinds, or menu behavior. | Keep appearance first, setup/community tools second, and overlay behavior third. |
...
Choose the tab where players will look for the feature first.

Supported integrations add `Speedrun Tool`, `CelesteTAS`, `Extended Variant Mode`, and `Extended Camera Dynamics` tabs when those mods are loaded. Keep integration-specific rows in the matching interop catalog instead of a base tab.
...
| Overlay row placement | `Source/Overlay/akron-overlay-entries.cs` for base rows, with focused catalogs in `Source/Overlay/akron-overlay-label-entries.cs`, `Source/Overlay/akron-overlay-keybind-entries.cs`, and `Source/Overlay/akron-overlay-interop-entries.cs` |
...
- Add an `Action(...)` row in the nearest owning overlay catalog.
...
Akron UI copy should describe observable behavior.
...
- Do not repeat classification text manually. The UI owns policy display.
...
- [ ] Policy classification and tests are updated when the feature affects classification.
