...
Akron changes should move from behavior to policy to implementation to verification. Start from the user-visible behavior instead of the UI label.
...
| `model_used` | The actual model used, or `not disclosed by tool` when the tool does not expose it. Do not write `unknown`, `default`, `auto`, `latest`, or a guessed model name. |
