...
`Unclassified` is the new or reset attempt state before Akron records feature use. Feature definitions use the other three values.
...
- Death Stats can be Goldberry/Hardlist clear while the PB loss prompt is Normal clear.
...
| Room names and status labels | `RegularClean` | Clean overlay information, but not strict-submission approved by default. |
...
| Custom labels | `Cheat` | Templates can expose exact player, resource, configuration, and run state. |
| Noclip, invincibility, infinite resources, and user-controlled non-default variants | `Cheat` | Gameplay mutation. |
| FPS Bypass without interpolation, TAS mode, or Nasty mode | `RegularClean` | Raises draw cadence while keeping Celeste physics at 60 TPS. |
| Pause/frame stepping, timescale, and TPS bypass | `Cheat` | Simulation timing or proof assumptions change. |
...
Tooltips describe observable behavior.
