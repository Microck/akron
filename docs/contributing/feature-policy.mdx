...
Akron feature policy classifies behavior by what it does to gameplay, proof assumptions, hidden information, and state. The policy source of truth is `Source/Core/AkronFeatureRegistry.cs`.
...
| `RegularClean` | Green | Normal clear for ordinary play, but not necessarily approved for strict clean submissions. |
| `Cheat` | Red | Changes gameplay, exposes hidden information, changes proof assumptions, or manipulates state. |
...
- [Public feature status reference](/reference/feature-status-reference).
...
- The registry, tests, feature reference, and this page agree on the classification rule.
