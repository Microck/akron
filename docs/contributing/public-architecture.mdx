...
Akron is organized around a small number of public concepts: overlay UI, persisted settings, `.akr` packs, policy gates, runtime services, capture/proof output, and tests.
...
| Settings | Stores user-configurable values and persisted setup state. Profile capture, profile application, HUD settings, defaults, clamps, display helpers, and recorder settings live in focused settings/profile partials. |
| Policy | Classifies features and records or blocks use under active rulesets. |
...
| `.akr` packs | Exports/imports scoped settings. Profile archive contracts and section copying live in the profile-pack service; OS-specific file picker process launching lives in a focused profile file-picker partial. |
