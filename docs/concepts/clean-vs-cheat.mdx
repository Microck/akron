...
Use this page when you want to understand why a feature carries a classification. Use the [Feature guide](/feature-guide) when you need the current classification for a specific overlay row.
...
A row's tooltip shows the registry classification of the feature it controls, and a popup option's tooltip shows its own. The attempt-status chip shows the strictest classification recorded for the current attempt. Showing a classification does not mean that row has already changed the attempt status.
...
| Disable Playback | Suppresses map-placed playback tutorial ghosts during gameplay. |
...
Check the exact option's tooltip. A popup option can be stricter than its parent row.
...
For players, read the tooltip for the exact option you are enabling.
