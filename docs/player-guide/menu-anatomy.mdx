...
| Submenu | The small angled triangle marker on the right side of a row. It indicates that the row opens more controls instead of only toggling or editing a value. | The marker on `FPS Bypass`, `Frame Stepper`, `Autosave`, or `Transition Speed`. |
