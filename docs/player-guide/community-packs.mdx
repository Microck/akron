...
- For StartPos packs, install the same map and object-providing mods used by the author. Each imported StartPos includes its v9 snapshot, coordinates, and spawn settings.
...
- A public pack contains `manifest.json`, `setup.json`, and one v9 snapshot entry per StartPos. Its manifest `kind` is `setup` and its payload format is `akron-setup-v6`.
