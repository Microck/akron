...
Catalog screenshots can start larger than the stored image. The official bot accepts capture sources up to 64 MiB, then stores an optimized WebP preview capped at 4 MiB so detailed captures can fit inside the public catalog without burning through storage.
