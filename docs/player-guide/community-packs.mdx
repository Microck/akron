...
Catalog screenshots can start larger than the stored image. Official upload paths store the source capture privately for review, then publish one optimized WebP preview for the catalog. In-game uploads automatically capture the full map before submission.

In-game uploads currently submit StartPos packs only. Auto Kill and Auto Deafen area lists are not uploaded until those area formats carry explicit map scope, because a leftover area from another map would otherwise be published under the current map.
