...
- **StartPos packs carry exact room state.** Each imported StartPos includes its v7 snapshot, coordinates, and spawn settings. The same map and object-providing mods must be installed.
...
- **Only setup archives import.** Public packs must use the current `.akr` setup archive shape: `manifest.json`, `setup.json`, and one v7 snapshot entry per StartPos, with `Kind` set to `setup` and setup format `akron-setup-v4`.
...
The official Discord bot must allow the same 512 MiB `.akr` limit before publishing a public catalog pack.
