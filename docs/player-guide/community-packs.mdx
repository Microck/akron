...
Catalog screenshots can start larger than the stored image. Official upload paths accept capture sources up to 100 MiB, then store an optimized WebP preview capped at 4 MiB so detailed captures can fit inside the public catalog without burning through storage. In-game uploads automatically capture the full map before submission.
