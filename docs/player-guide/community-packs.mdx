...
- **StartPos packs carry exact room state.** Each imported StartPos includes its v6 snapshot, coordinates, and spawn settings. The same map and object-providing mods must be installed.
...
- **Only setup archives import.** Public packs must use the current `.akr` setup archive shape: `manifest.json`, `setup.json`, and one v6 snapshot entry per StartPos, with `Kind` set to `setup` and setup format `akron-setup-v3`.
