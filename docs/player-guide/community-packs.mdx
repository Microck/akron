...
The browser reads packs from a JSON catalog at the configured **Index URL** (shown at the top of the Community Packs window). The default catalog is hosted at `https://akron.micr.dev/catalog/index.json`. You can point this to a custom HTTPS URL or explicit local `file:///` path. Catalog entries can contain map-specific **StartPos**, **Auto Kill**, or **Auto Deafen** `.akr` packs.
...
- For StartPos packs, install the same map and object-providing mods used by the author. Each imported StartPos includes its v7 snapshot, coordinates, and spawn settings.
...
- A public pack contains `manifest.json`, `setup.json`, and one v7 snapshot entry per StartPos. Its manifest `kind` is `setup` and its payload format is `akron-setup-v4`.
