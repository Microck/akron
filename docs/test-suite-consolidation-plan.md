...
2. Replace the source predicate in `RetainedFullCollectionsReconcileDebtOnlyAfterTheyRun` with Cecil instruction fixtures for `call` and `callvirt` that invoke the real reconciliation manipulators and inspect the inserted instruction order. The `callvirt` fixture tests only the matcher's current acceptance of that instruction shape: `GC.Collect()` is static, so a `callvirt` targeting it is invalid executable IL and must not be executed or presented as runtime coverage. Use valid static `call` fixtures for executable checks that debt reconciliation follows the actual collection. Keep this work in the existing IL test owner; no new IL-testing framework is needed. This replaces the source-text dependency while distinguishing opcode recognition from runtime ordering.
