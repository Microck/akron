...
In-game uploads go to the Upload Worker first. The Worker stores source `.akr` files and full-map captures in a private quarantine bucket, records batch, submission, and object state in its database, and exposes signed bot-only endpoints for moderation. Nothing from quarantine is public.

The Discord bot polls the Worker for queued jobs, posts moderation messages, handles attribution confirmation, and sends approve, reject, or request-changes actions back to the Worker. Approval is the only path that copies a pack into public catalog storage.

When a submission is approved, the Worker writes the `.akr` archive to the public pack path, transforms the capture into one catalog preview image, stores that preview at the public capture path, and rebuilds `catalog/index.json`. The public website serves branded Akron URLs and routes the reserved catalog and map asset paths to the public storage objects.
