...
- Each pack declares no more than 16 preview images.
...
- Verify that the archive contains `manifest.json`, `setup.json`, and the exact v6 snapshot entry named by each StartPos.
- Verify that the manifest `Kind` is `setup`.
- Ensure that `setup.json` uses the `akron-setup-v3` format.
