...
- Verify that the archive contains `manifest.json`, `setup.json`, and the exact v8 snapshot entry named by each StartPos.
- Verify that the manifest `kind` is `setup`.
- Ensure that `setup.json` uses the `akron-setup-v5` format.
