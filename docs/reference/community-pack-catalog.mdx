...
- Verify that the archive contains `manifest.json`, `setup.json`, and, when StartPos slots are present, `startpos/snapshots.bin.br` containing each declared v11 snapshot.
- Verify that the manifest `kind` is `setup`.
- Ensure that `setup.json` uses the `akron-setup-v10` format.
