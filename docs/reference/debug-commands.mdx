...
| `akron_room_capture <start\|stop\|status\|format <png\|jpg>\|markers <on\|off>\|startpos <on\|off>\|autokill <on\|off>\|autodeafen <on\|off>\|removebg <on\|off>\|removefg <on\|off>\|wait <frames>\|horizontal <tiles>\|vertical <tiles>>` | Drive Room Capture. The overlay Room Capture row is the normal player-facing path. |
| `akron_map_capture <start\|stop\|status\|format <png\|jpg>\|markers <on\|off>\|startpos <on\|off>\|autokill <on\|off>\|autodeafen <on\|off>\|removebg <on\|off>\|removefg <on\|off>\|wait <frames>\|horizontal <tiles>\|vertical <tiles>>` | Drive Map Capture. The overlay Map Capture row is the normal player-facing path. |
| `akron_startpos <status\|slot <n>\|set\|load\|clear\|prev\|next\|place <on\|off>\|dashes <n>\|stamina <n>\|slots <n>\|facing <current\|left\|right>\|idle <on\|off>\|grab <on\|off>\|label <on\|off>\|respawn <on\|off>>` | Full StartPos control. `akron_save` and `akron_load` provide convenience shortcuts. |
| `akron_slot <n>` | Set or show the active StartPos slot (1-9). |
...
| `akron_auto_kill speed <on\|off>`, `min-speed <n>`, `max-speed <n>` | Require a total speed range for area-based Auto Kill. |
| `akron_auto_kill h-speed <on\|off>`, `min-h-speed <n>`, `max-h-speed <n>` | Require a horizontal speed range for area-based Auto Kill. |
| `akron_auto_kill v-speed <on\|off>`, `min-v-speed <n>`, `max-v-speed <n>` | Require a vertical speed range for area-based Auto Kill. |
| `akron_auto_kill horizontal <any\|left\|right\|still>` | Require a horizontal movement direction for area-based Auto Kill. |
| `akron_auto_kill vertical <any\|up\|down\|still>` | Require a vertical movement direction for area-based Auto Kill. |
| `akron_auto_kill dash-count <on\|off>`, `dashes <n>` | Require a current dash count for area-based Auto Kill. |
| `akron_auto_kill ground <any\|grounded\|airborne>` | Require a grounded or airborne state for area-based Auto Kill. |
| `akron_auto_kill state <on\|off>`, `state-id <n>` | Require an internal player state id for area-based Auto Kill. |
| `akron_auto_kill invert <on\|off>` | Trigger area-based Auto Kill when enabled conditions fail instead of when they pass. |
