...
Akron uses `.akr` files to share structured setup data. An `.akr` file is a ZIP archive containing one manifest and one main payload. StartPos setup packs also contain the exact compressed snapshot named by each StartPos entry. The reader rejects every file that the payload does not declare.
...
flowchart TD
  ...
  archive --> snapshots["startpos/*.v6.json.gz"]
...
Each expanded v6 snapshot is limited to 192 MiB while Akron validates it.
...
active save-file slot. The attachment carries room and session state, not the
author's global save progress or module save data.
