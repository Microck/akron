...
public static partial class AkronCommands {
    [Command("akron_frame_bypass", "control frame bypass: status|fps on/off|fps-target <60-480>|tps on/off|tps-target <30-480>|method interval|dynamic|camera fancy|fast|off|objects extrapolate|interpolate|tas on/off|subpixel on/off|background on/off|foreground on/off|hide-edges on/off|silly on/off")]
    public static void FrameBypass(string action = "status", string value = "") {
        ...
        switch (normalized) {
            ...
            case "silly":
            case "sillymode":
                if (!TryParseBoolean(value, out bool sillyMode)) {
                    Log("usage: akron_frame_bypass silly on|off");
                    return;
                }
                ...
                return;
            default:
                Log("usage: akron_frame_bypass status|fps on/off|fps-target <60-480>|tps on/off|tps-target <30-480>|method interval|dynamic|camera fancy|fast|off|objects extrapolate|interpolate|tas on/off|subpixel on/off|background on/off|foreground on/off|hide-edges on/off|silly on/off");
                return;
        }
    }
