...
using Celeste.Mod.Akron.MotionSmoothing.Smoothing.States;
using Celeste.Mod.Akron.MotionSmoothing.Smoothing.Strategies;
using Celeste.Mod.Akron.MotionSmoothing.Utilities;
using Microsoft.Xna.Framework;
using Monocle;

namespace Celeste.Mod.Akron.MotionSmoothing.Smoothing;
...
public class MotionSmoothingHandler : ToggleableFeature<MotionSmoothingHandler>
{
    ...
    public void ResyncCurrentLevelObjects()
    {
        if (!Enabled || Engine.Scene is not Level) return;

        // Native StartPos restores copy a saved room graph back into the live Level.
        // That can leave smoothing history pointing at pre-restore objects and camera
        // positions even after the logical camera has been corrected. Rebuild the
        // registry from the current Level and seed every history from the live values
        // before the next draw.
        SmoothAllObjects();
        ValueSmoother.UpdatePositions();
        PushSpriteSmoother.UpdatePositions();
        _positionsWereUpdated = true;
