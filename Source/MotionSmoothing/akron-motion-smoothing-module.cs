...
using Celeste.Mod.Akron.MotionSmoothing.FrameUncap;
using Celeste.Mod.Akron.MotionSmoothing.Interop;
using Celeste.Mod.Akron.MotionSmoothing.Smoothing;
using Celeste.Mod.Akron.MotionSmoothing.Smoothing.Targets;
...
public sealed class MotionSmoothingModule {
    public const BindingFlags AllFlags =
        BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static;

    public static MotionSmoothingModule Instance { get; } = new MotionSmoothingModule();

    public static MotionSmoothingSettings Settings { get; } = new MotionSmoothingSettings();
    ...
    private void ApplyFramerate()
    {
        ...
        int drawFps = Settings.Enabled ? rates.DrawRate : 60;
        double updateFps = Settings.Enabled ? rates.UpdateRate : 60.0;
        ...
        if (!InLevel)
        {
            ...
            if (Settings.TasMode) {
                drawFps = 60;
                updateFps = 60;
            } else if (Settings.GameSpeedModified && !Settings.GameSpeedInLevelOnly) {
                updateFps = Settings.GameSpeed;
            }

            FrameUncapStrategy.SetTargetFramerate(updateFps, drawFps);
            if (DecoupledGameTick.Enabled && !AkronModule.Settings.TpsBypass && (Settings.TasMode || !Settings.GameSpeedInLevelOnly))
        ...
        FrameUncapStrategy.SetTargetFramerate(updateFps, drawFps);
        if (DecoupledGameTick.Enabled && !AkronModule.Settings.TpsBypass)
            DecoupledGameTick.SetTargetDeltaTime(60);
    }
