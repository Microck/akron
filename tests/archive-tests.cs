...
public sealed class ArchiveTests : IDisposable {
    ...
    public void PayloadArchiveRoundTripsDeclaredBinaryAttachment() {
        ...
        AkronArchive.WritePayloadArchive(
            ...
            "setup.json",
            "{}",
            new Dictionary<string, string> { ["startpos/1.v8.json.gz"] = attachmentPath });
        ...
        Assert.Equal(new[] { "startpos/1.v8.json.gz" }, attachmentNames);
    ...
    public void PayloadArchiveRejectsTooManyAttachmentsOnRead() {
        ...
        using (ZipArchive archive = ZipFile.Open(path, ZipArchiveMode.Create)) {
            ...
            WriteEntry(archive, "startpos/1.v8.json.gz", "one");
            WriteEntry(archive, "startpos/2.v8.json.gz", "two");
        }
    ...
    public void PayloadArchiveRejectsOversizedAttachmentSetOnRead() {
        ...
        using (ZipArchive archive = ZipFile.Open(path, ZipArchiveMode.Create)) {
            WriteEntry(archive, "manifest.json", JsonSerializer.Serialize(
                ...
                new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.CamelCase }));
            WriteEntry(archive, "setup.json", "{}");
            WriteEntry(archive, "startpos/1.v8.json.gz", "12345");
            WriteEntry(archive, "startpos/2.v8.json.gz", "67890");
        }
