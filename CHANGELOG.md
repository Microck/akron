...
- Say whether a StartPos slot's saved state has reached the disk yet, in `akron_status` and in `akron_startpos status`. Setting a slot has never put it there: the copy that lets a slot survive a restart runs in the background, and this release parks it while you are in control of the game rather than letting it run there. On the Windows test machine that gap reached twenty-five minutes of continuous play with three slots set and nothing written, and the only thing anyone could read said the slots were set. Three lines say it now: `startpos-snapshot-on-disk` and `startpos-restart-copy-outstanding` for the slot in front of you, and `startpos-restart-copies-outstanding` for how many Sets across the whole session are still waiting for their copy. They report the file and the queue rather than passing a verdict on the slot: whether a saved state loads is decided by reading it, which only a load does. `startpos-set` keeps the meaning it had, which is that the slot is in the list for this map. Nothing about when a copy runs changes. Closing the game normally still spends a few seconds on the copies that are outstanding rather than all the time they would need. What these lines make visible is the window in which a crash or a kill takes a slot with no message at all.
...
- Refuse another StartPos Set while that map still holds a measured clone above the 1 GiB warm-memory limit, instead of repeating a known oversized capture and risking process memory exhaustion. Clearing the oversized slot removes that projection, so it does not block later smaller captures.
- Start each performance JSONL recording with a clean sample window and write its final short window when recording stops, so the file contains no frames from before the command and does not lose its last 1-119 frames.
- Keep a backup that missed files or saw the save-file list change from counting as the latest successful backup or pruning a complete one. Akron keeps the newest incomplete ZIP for manual recovery, removes incomplete ZIPs before restorable backups when retention reaches a limit, and still prunes old Akron-format backups while leaving unrelated ZIPs alone.
- Remove a deleted save profile's StartPos files and cached read-ahead data, and scope the cache to the profile incarnation so recreating the same numbered slot cannot retain the deleted profile's disk or memory use.
...
- Restore StartPos in rooms with groups of linked mod entities, such as Spring Collab 2020's Ancient Engine. Checked in game on that map: eight slots, one per room, including the three rooms carrying ten or eleven linked clutter blocks. After a full restart all eight loaded from disk.
