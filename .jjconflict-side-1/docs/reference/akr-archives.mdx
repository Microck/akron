...
Setup packs use the `profile` archive contract:
...
The `profile.json` payload uses the following structure:
...
- Must be defined as a field in `AkronProfileState`.
...
New archive kinds are reserved for payloads that are not setup/profile sections.
