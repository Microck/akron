...
<Steps>
  ...
  <Step title="Write the template">
    Set **Text** to plain text plus variables in braces, such as `Room {room_time}` or `{profile} / {ruleset} / {status}`.
  </Step>
...
| `{profile} / {ruleset} / {status}` | Show the active Akron profile, ruleset stack, and attempt status. |
...
| `profile` | Active Akron profile. |
| `ruleset` | Active ruleset stack description. |
...
Custom label packs are separate from whole-profile `.akr` packs. A HUD-scoped profile export also includes custom HUD label settings as part of the broader HUD section.
...
See [Labels](/feature-guide/labels) for the complete Labels tab row reference and [Import and export .akr packs](/akr-packs/import-export) for profile pack workflows.
